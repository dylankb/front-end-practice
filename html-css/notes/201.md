...
Also vertical-align: middle does not work with `display: inline-block` elements.
...
* Margin - Works horizontally (`left` and `right`)  
* Padding
  - Horizontal padding  
...
* Able to take `vertical-align` property values 
  * `vertical-align` can also take px values.
...
* Flow-overlap/collapse: A floated element's unfloated parent elements in a container will overlap the dimenstions of the floated element. This is why a floated element collapses it's parent element to a size of nothing.
...
If there is some white space surrounding the columns then this layout is serviceable. The whitespace is necessary because the left width is a percent, so the right width is not totally fixed.
...
**Example with fixed right column:**
http://codepen.io/dylankb/pen/GqvLLO

This is also reversible (switch the left hand column to a fixed width) without having to change the HTML structure.
...
* `left` property pushes right, etc.
...
* Removed from document layout, sticks to that location
...
With a `left: 50%` value you would then `margin-left:` value equal to 1/2 of the elements width
...
This is because if the `absolute` element has no positioned parent elements, then it will be absolutely positioned relative to the body element. To center the `.positioned` relative to `section`, `.positioned` must become a child of `section` in the markup and `section` must take a position (probably `relative`) value.
...
2) Image - position or varied height

To select a specific image, move down y-axis with negative values equal to the height of that image. If image height is 124px then the image next from the top should be: 
...
Psuedo selectors need a `content` property: Empty string can work `""`

Also will need `display: block` or `inline-block` since by default they are `display: inline`. `inline` it can't take width or height dimensions, making the height and weight values effectively zero*
...
Cool course, here's the feedback you all requested on our experience taking the course.

Things that worked

- Learning chrome developer tools
- Hands on challenges (build this, fix this, etc.)
- Focused exercises
...
- How to clear floats!
- The box model
- A better feel and understanding of padding, margin, and their use cases.

Things that I had to review again

Generally if it wasn't required to do in multiple projects things that were not common themes throughout multiple projects I probably forgot it :)

- Spritesheets: I think it was only in one project, so I didn't remember how these worked.
- Positioning (absolute and relative)
- background-images
- More complicated shorthands: i.e background-image


Things I had to look up

- Some of the "gnitty-gritty" about what CSS is doing behind the scenes. For example why floats collapse their parent elements/containers.
- Tips for styling tables. The curriculum indirectly covered this portion with a project. I think having students specifically read the Shay Howe tutorial and complete the exercises before the Nutrition project would be a good idea.
- Edge cases in centering. We learn how to deal with block elements in a container, vertical alignment with inline-elements, but there are more difficult that come up during the course. I had to look at articles like this https://css-tricks.com/centering-css-complete-guide/

As someone who did not pass the take home test portion of the HTML/CSS I may have some insight here.

I think there's some good guidance about what you'll need to be able to demonstrate for the interview, but I was a little suprised by the difficulty and level of standards for the project (maybe I shouldn't have been, but I was). Anyways, a little more guidance around this pre-test area could be good. For example, suggest that they try doing a previous project and if they aren't sure what all effects of all their code then they suggest some further study prep activities.

- I have never met a senior Ruby developer who said he understood CSS well ( or thought it was important they learn )
- I also spoke with a front-end/designer dev who didn't think CSS would very important even if I was going for entry level full-stack positions. They said that the trend is for front-end developers who have a design background (or designers themselves) work with CSS in depth.
- I listen to development podcasts and regularly hear senior developers admit they don't have a thorough understaninding of CSS.
  - Thoughtboters Chris Tumi and Ben Orenstein admitted they could not center things with CSS. 
  - Jessica Kerr on Ruby Rogues pick to share with the audience last month was a book about HTML5 (she wasn't familiar with HTML5 beforehand). She speaks widely at conferences and works at Stripe as a developer.
 
- Spent an extra few months after doing the first Tea Leaf course to go through LS, and I'm glad I spent that extra time on Ruby, OOP, and web dev. 
- Can't say that knowing felt worth it.

- A good foundation in CSS is of course a nice to have. 

- At the risk of being labeled someone who "doesn't have the right attitude" I would say just completing the projects for this course is enough unless you want to be a designer front end developer. 

- For some people CSS doesn't click, on spending the extra time to try and get it to might not be worth it.
- I had to write over 6000 words of notes, which is almost 12 pages if it were a college paper :)

- Lots of discontinuity: Once you start to get a handle on web development you completely switch gears.
- I would go 100 - 300 - 200 if possible

- Or make the course optional

Anyways, I wanted to pass the assessment before sending this on so this didn't come off as me being lazy or wanting a special exemption.

More specific thoughts aspects of the course:

I think one thing you're supposed to do early on in the HTML course (or front end prep course?) is to read the Shay Howe book cover to cover. 

Looking in the lessons I don't see this an early requirement, and if that's the case I'd agree with that move because it seemed too in depth to to read the whole thing at at once early on. The projects were great, but not for someone just learning.

It's a good book to:

1) Throw in the odd chapter as required reading per lesson. For example, the tables lesson on tables. It covers styling tables, and I don't remember the curriculum directly teaching this in a lesson.
2) Suggestion students read the book cover to cover before their assessment as review.

Video content

The videos, and to a lesser extent some of the writing in the lessons, especially at the very beginning feelt a little bit rushed at times.

Shane seems like a talented front-end developer, and I learned a lot about front-end development from his course. One thing I would point out is that Launch School instructors and assessments really hammer home the need to be able to explain the fundamentals of how a language works, but the 201 videos generally passed on a lot of opportunities for "in-depth explanation" on the HTML or CSS code that they were using. The new instructor for course 170 was particularly good at explaining the code he was writing. 

I know for a fact that coding and explaining concepts or trade-offs in your implementation can be difficult to do at the same time, however it's very useful when done well for two big reasons. One, this helps you talk about the code. It's important to be able to verbalize why something works and not just be able to say "I get it". A great way to learn how to verbalize a concept is hear a real developer talk about it. Two, it helps me apply the concepts the next time. Perhaps the best thing about assigment walk-throughs are the chance to hear a developer's thought process and resoning behind their implementation broken down step-by-step. The next time I have to make a layout decision, if I've already heard the benefits and drawbacks a couple times I'm able to draw on those nuances to make an intelligent implementation decision.

Basically I saw a lot of passed over opportunites to explain how HTML and CSS elements behave and interact in the wild, what's best to avoid, patterns to look for, etc. A simple exampe would be early on instead of saying "Let's apply a margin", expaling a bit why you're using margin instead of padding, why you're using it on that particular element,etc.

To be fair, it seems in later videos the instructor did start talking more strategy when putting together a web-page.

Photoshop 

People I know who do design say Photoshop isn't as common to use as it once was, and were a bit perplexed that I was learning it. They also said it was a bit overpowered for web design.

Curriculum ideas: 

1) Summaries or CSS behavior quizzes: Most of the important CSS concepts are explained in the lessons, but sometimes it's a little hard to find this information later on if you forgot something so a nice summary important concepts per lesson is useful. For example, why a float collapses its parent container. This was basically explained in a lesson, but I didn't remember it being there so I looked up that behavior in CSS tricks. In course 170 I read all the summary pages from the end of each lesson and took some notes before my assessment and I felt like I had a good feel for everything we'd covered. Fill in the blank quizzes on CSS behaviors could help do the same thing.

2) Shortcuts: Adding a quick section early on referencing some helpful Sublime/Atom/Vim(?) shortcuts for writing markup more quickly. For example, even knowing something simple like Sublime's tab-completion feature makes coding tags much faster.

3) More practice manipulating background-images. We had some practice at doing this during the mobile breakpoint exercise for the company site, but some of the sizing considerations were a bit tricky to remember and it would nice to have these techniques broken down a bit more in a lesson.

4) Include a video walkthrough for the nutrition label project. This is more or less the only practice in the curriculum where you need to style a table, so it would be good to hear more about different considerations/tactics/rules etc. when you try and do this.

Random UX ideas.

These aren't super important, but since I'm already writing down feedback here you go:

1) Easier access to discussions page: I would love to be able to click into discussions from each lesson content page I'm currently going through. Currently you can get to the discussions from the general select lesson page, and you are taken by default to the discussions when you click into that course, but when I actually want to see the discussions are when I'm stuck and want to see if anyone else had a similar issue. It doesn't have to be integrated search or anything fancy, a link on the top of the page would do.

I can understand showing the 'Discussions' tab by default when you click into a lesson as a way to get more people involved, but it adds a lot of clicks to the process of trying to find the right page in a lesson and this could be an alternative.

2) Video controls: Great job adding the video play speed controls! It would be great to have a jump forward and backwards 10/15 seconds buttons on the browser video player as well, though.

Program

1) It would be great to have post-work office hours for those of us who work full-time.

2) Assessments

Why limit the number of fails?

Adds unnecessary stress

Stress -Unnecessary stress for the student (although some is good)

Abandoning - I understand if they fail the first assessment 6 times, maybe the program isn't right for them. But what if they had invested 6-8 months and are kicked out, that's a very unfortunate situation.

Responsibility - Failing is good feedback for you. Pressure shouldn't just be offloaded onto the student. There's accountability on both ends of this thing.

I.e. the curriculum should be that if sometimes takes the courses very seriously they should be able to pass at the end.

*****
##Interview prep

During the interview, you should be able to clearly explain the following concepts on CSS:
...
Most of the questions will also need you to give an example implementation on the fly, and we'll be looking for both accuracy and fluency.

**Things to practice - interview**

1. Give an example of each concept beforehand, and check for accuracy

**Examples to code**

3) Floating a second element to the boundary of the parent (or page).  
- Useful for having related pairs lined where the first element is variable in length

...
* `left` property pushes right, etc. (like using padding-left to move something right).
* affects layout of surrounding elements
* Block style width

Here's is basically the same CSS from the `position: absolute` example, except with the value changed to `relative`.
